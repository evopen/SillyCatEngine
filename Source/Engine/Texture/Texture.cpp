...
namespace Sce
{
    ...
    {
        int channels;
        uint8_t* data = stbi_load_from_memory(static_cast<uint8_t*>(inBuffer), static_cast<int>(len), reinterpret_cast<int*>(&Extent.width), reinterpret_cast<int*>(&Extent.height), &channels, STBI_rgb_alpha);
        ImageData.resize(Extent.width * Extent.height * STBI_rgb_alpha);
        std::memcpy(ImageData.data(), data, Extent.width * Extent.height * STBI_rgb_alpha);
        stbi_image_free(data);
        Extent.depth = 1;
